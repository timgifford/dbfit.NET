...
public class TestData extends ColumnFixture {
	...
	protected void performOperation(DatabaseOperation operation,
		...
		try {
			conn = dbEnvironment.getConnection();
			operation.execute(getDatabaseConnection(conn), replacement);
		} finally {
			if (conn != null) {
				try {
					conn.close();
	...
	protected IDatabaseConnection getDatabaseConnection(Connection jdbcConnection)
			throws Exception {
